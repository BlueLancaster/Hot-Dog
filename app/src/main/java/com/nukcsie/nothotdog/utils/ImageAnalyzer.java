...
public class ImageAnalyzer implements ImageAnalysis.Analyzer {
    ...
    public void analyze(@NonNull @NotNull ImageProxy image) {
        try {
            HotDogModel hotDogModel = HotDogModel.newInstance(context); // 建立模型
            ImageProcessor imageProcessor = new ImageProcessor.Builder()    // 建立圖片處理器
                    .add(new ResizeOp(224, 224,
                            ResizeOp.ResizeMethod.BILINEAR)).build();
            TensorImage tensorImage = TensorImage.fromBitmap(toBitmap(image));  // 將圖片轉為TensorImage類別
            tensorImage = imageProcessor.process(tensorImage);  // 再經過處理
            outputBuffer = hotDogModel.process(tensorImage.getTensorBuffer())   // 放入模組執行並回傳結果
    ...
    private Bitmap toBitmap(@NotNull ImageProxy imageProxy) {   // 轉圖片為bitmap
    ...
    private void initializeRecognitionItems() throws IOException {  // 讀取txt檔得知分析的結果的可能種類
